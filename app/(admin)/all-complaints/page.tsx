...
import { getComplaints, updateComplaintStatus, assignComplaint, Complaint, getCurrentUser, User } from "@/lib/store";
...
export default function AllComplaints() {
  ...
  const getFilteredComplaints = () => {
    ...
    if (user?.role === "officer") {
      // For this prototype, an officer sees complaints assigned to their username, 
      // or to the generic 'officer_demo' mock assignment.
      list = list.filter(c => 
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "Pending": return "bg-error-container/50 border-error-container text-on-error-container focus:ring-error";
      case "In Progress": return "bg-orange-50 border-orange-200 text-orange-700 focus:ring-orange-500";
      case "Resolved": return "bg-green-50 border-green-200 text-green-700 focus:ring-green-500";
      default: return "bg-surface-variant border-outline-variant text-on-surface-variant focus:ring-outline";
    }
  };
  ...
  return (
    <main className="flex-1 p-margin-mobile md:p-margin-desktop w-full max-w-container-max mx-auto pb-32 md:pb-margin-desktop relative">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-on-surface/50 backdrop-blur-sm p-4">
          <div className="bg-surface rounded-xl shadow-lg max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b border-outline-variant pb-4 mb-4">
              ...
              <button onClick={() => setViewComplaint(null)} className="p-2 hover:bg-surface-container rounded-full transition-colors">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-surface-container-low p-4 rounded-lg">
                <p className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-1">Citizen / Applier</p>
                <p className="font-sans text-body-md font-semibold text-on-surface">{viewComplaint.citizenName}</p>
              </div>
              <div className="bg-surface-container-low p-4 rounded-lg">
                <p className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-1">Category</p>
                <p className="font-sans text-body-md font-semibold text-on-surface">{viewComplaint.category}</p>
              </div>
              <div className="bg-surface-container-low p-4 rounded-lg">
                <p className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-1">Ward</p>
                <p className="font-sans text-body-md font-semibold text-on-surface">{viewComplaint.ward}</p>
              </div>
              <div className="bg-surface-container-low p-4 rounded-lg">
                <p className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-1">Status</p>
                <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${getStatusColor(viewComplaint.status)}`}>
            ...
            <div className="mb-6">
              <p className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-2">Description</p>
            ...
            <div>
              <p className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-2">Timeline Updates</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-on-surface/50 backdrop-blur-sm p-4">
          <div className="bg-surface rounded-xl shadow-lg max-w-sm w-full p-6">
            <div className="flex justify-between items-start mb-4">
              <h2 className="font-heading text-title-md font-bold text-on-surface">Assign Officer</h2>
              <button onClick={() => setAssignModalData(null)} className="p-1 hover:bg-surface-container rounded-full transition-colors">
                <X className="w-5 h-5 text-on-surface-variant" />
              </button>
            </div>
            <p className="font-sans text-body-md text-on-surface-variant mb-4">
              Assign complaint <strong>{assignModalData.id}</strong> to a department officer.
            </p>
            <div className="mb-6">
              <label className="block font-sans text-label-sm text-on-surface-variant mb-2">Select Officer</label>
              <select 
                value={selectedOfficer}
                onChange={(e) => setSelectedOfficer(e.target.value)}
                className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-4 py-3 focus:outline-none focus:border-primary focus:ring-1 font-sans text-body-md"
              >
                <option value="">-- Choose Officer --</option>
                <option value="Santosh K.">Santosh K. (Solid Waste)</option>
                <option value="Vijay E.">Vijay E. (Streetlights)</option>
                <option value="Ramesh T.">Ramesh T. (Water Supply)</option>
                <option value="officer_demo">Officer Demo Account</option>
              </select>
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => setAssignModalData(null)} className="px-4 py-2 text-on-surface-variant font-sans font-semibold rounded-lg hover:bg-surface-container-low">Cancel</button>
              <button onClick={handleAssign} disabled={!selectedOfficer} className="px-4 py-2 bg-primary text-on-primary font-sans font-semibold rounded-lg hover:bg-primary-container disabled:opacity-50">Assign Task</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-stack-lg gap-stack-md pt-4 md:pt-0">
        <div>
          <h2 className="font-heading text-headline-lg-mobile md:text-headline-lg text-primary mb-2 font-bold">All Complaints</h2>
          <p className="font-sans text-body-md text-on-surface-variant">Central management console to monitor, assign, and update all registered civic issues.</p>
        </div>
      </div>
      ...
      <div className="bg-surface border border-outline-variant rounded-xl overflow-hidden shadow-sm">
                ...
                filteredComplaints.map((complaint) => (
                  <tr key={complaint.id} className="hover:bg-surface-container-lowest transition-colors group">
                    ...
                    <td className="py-4 px-6">
                      ...
                      <div className="text-label-sm text-on-surface-variant truncate max-w-[200px]">{complaint.description}</div>
                    </td>
                    <td className="py-4 px-6">{complaint.ward}</td>
                    <td className="py-4 px-6">
                      {isMayor ? (
                        <span className={`inline-block border text-label-sm font-semibold rounded-full px-3 py-1 text-center ${getStatusColor(complaint.status)}`}>
                          {complaint.status}
                        </span>
                      ) : (
                        <select 
                          ...
                          className={`border text-label-sm font-semibold rounded-full px-3 py-1 focus:outline-none focus:ring-1 appearance-none cursor-pointer text-center ${getStatusColor(complaint.status)}`}
                        >
                          <option value="Pending">Pending</option>
                          <option value="In Progress">In Progress</option>
                          <option value="Resolved">Resolved</option>
                    ...
                    <td className="py-4 px-6">
                      {complaint.assignedTo && complaint.assignedTo !== "Assign" ? (
                        ...
                        isAdmin ? (
                          <button onClick={() => setAssignModalData(complaint)} className="flex items-center gap-1 text-primary hover:text-primary-container text-label-sm font-medium bg-primary/5 px-2 py-1 rounded">
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => setViewComplaint(complaint)} className="p-1.5 text-on-surface-variant hover:text-primary hover:bg-surface-container-low rounded transition-colors" title="View Details">
                          <Eye className="w-4 h-4" />
                        </button>
                        {isAdmin && (
                          <button className="p-1.5 text-on-surface-variant hover:text-primary hover:bg-surface-container-low rounded transition-colors" title="Edit">
        ...
        <div className="p-4 border-t border-outline-variant flex justify-between items-center bg-surface-container-low text-label-sm text-on-surface-variant">
          ...
          <div className="flex gap-1">
            <button className="px-3 py-1 rounded border border-outline-variant bg-surface hover:bg-surface-container-high transition-colors disabled:opacity-50" disabled>Previous</button>
            <button className="px-3 py-1 rounded border border-outline-variant bg-surface hover:bg-surface-container-high transition-colors">Next</button>
          </div>
        </div>
      </div>
    </main>
