  ...
  return (
    <main className="flex-1 w-full max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-stack-md flex flex-col gap-stack-lg mb-20 md:mb-0">
      ...
      <div className="flex flex-col gap-2 pt-4 md:pt-0">
        ...
        <p className="font-sans text-body-md text-on-surface-variant">View and track the status of your submitted complaints and requests.</p>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-stack-md">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 mb-4">
             <input
              ...
              />
              <button type="submit" className="bg-primary text-on-primary px-6 py-3 rounded-lg hover:bg-primary-container hover:text-on-primary-container transition-colors font-sans text-label-md font-semibold flex items-center gap-2">
          ...
          {trackedComplaint ? (
            <div className="bg-surface border border-outline-variant rounded-xl p-6 flex flex-col gap-stack-md shadow-sm">
              <div className="flex justify-between items-start border-b border-outline-variant pb-4">
                <div className="flex flex-col gap-1">
                  <span className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider font-semibold">Active Complaint</span>
                  <h2 className="font-heading text-title-md text-on-surface font-bold">{trackedComplaint.category}</h2>
                  <p className="font-sans text-body-md text-on-surface-variant mt-1 font-medium">Ticket ID: {trackedComplaint.id}</p>
                </div>
                <span className={`inline-flex items-center px-3 py-1 rounded-full font-sans text-label-sm font-bold ${getStatusColor(trackedComplaint.status)}`}>
              ...
              <div className="py-4 pl-4 border-l-2 border-outline-variant ml-4 flex flex-col gap-8 relative">
                {trackedComplaint.timeline.map((event, index) => {
                  ...
                  return (
                    <div key={index} className="relative">
                      ...
                      <div className="flex flex-col gap-1">
                        <span className={`font-sans text-label-md font-bold ${isLast && !isResolved ? "text-primary" : "text-on-surface"}`}>
                          {event.status}
                        </span>
                        <span className="font-sans text-label-sm text-on-surface-variant">{event.date}</span>
                        <p className={`font-sans text-body-md text-on-surface-variant mt-2 bg-surface-container-low p-3 rounded-md ${isLast && !isResolved ? "border border-primary-container/30" : ""}`}>
                          {event.note}
                        </p>
                      </div>
                    </div>
                ...
                {trackedComplaint.status !== "Resolved" && (
                  <div className="relative opacity-50">
                    <div className="absolute -left-[27px] top-0 w-6 h-6 rounded-full bg-surface border-2 border-outline-variant flex items-center justify-center">
                    </div>
                    <div className="flex flex-col gap-1">
              ...
              <div className="flex gap-4 mt-4 pt-4 border-t border-outline-variant">
                <button className="px-6 py-2 bg-primary text-on-primary font-sans text-label-md rounded-md hover:bg-primary-container hover:text-on-primary-container transition-colors font-semibold">Add Note</button>
                <button className="px-6 py-2 border border-outline-variant text-primary font-sans text-label-md rounded-md hover:bg-surface-container-low transition-colors font-semibold">Contact Support</button>
              </div>
            </div>
          ) : currentId ? (
            <div className="bg-surface border border-outline-variant rounded-xl p-12 flex flex-col items-center justify-center text-center shadow-sm">
              ...
              <p className="font-sans text-body-md text-on-surface-variant max-w-sm">No complaint was found with ID "{currentId}". Please check and try again.</p>
            </div>
          ) : (
            <div className="bg-surface border border-outline-variant rounded-xl p-12 flex flex-col items-center justify-center text-center shadow-sm">
              <Search className="w-12 h-12 text-outline-variant mb-4" />
              <h3 className="font-heading text-title-md text-on-surface font-semibold mb-2">No Complaint Selected</h3>
              <p className="font-sans text-body-md text-on-surface-variant max-w-sm">Enter your Ticket ID above to view the tracking status of your complaint.</p>
            </div>
          )}
        </div>
          ...
          <div className="flex flex-col gap-4">
            {complaints.length > 0 ? complaints.map(complaint => (
              <div 
                ...
                className={`bg-surface border ${currentId === complaint.id ? 'border-primary' : 'border-outline-variant'} rounded-lg p-4 hover:shadow-sm transition-shadow cursor-pointer`}
              >
                <div className="flex justify-between items-start mb-2">
                  <span className="font-sans text-label-sm text-on-surface-variant font-semibold">{complaint.id}</span>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full font-sans text-label-sm font-semibold ${getStatusColor(complaint.status)}`}>
                    {complaint.status}
                  </span>
                </div>
                <h4 className="font-sans text-label-md text-on-surface mb-1 font-semibold">{complaint.category}</h4>
                <p className="font-sans text-body-md text-on-surface-variant text-sm mb-3 line-clamp-2">{complaint.description}</p>
                <div className="flex justify-between items-center pt-3 border-t border-outline-variant">
                  ...
                  {complaint.status === "Resolved" && (
                    <button className="text-primary font-sans text-label-sm hover:underline font-semibold" onClick={(e) => e.stopPropagation()}>Provide Feedback</button>
                  )}
                </div>
              </div>
            )) : (
